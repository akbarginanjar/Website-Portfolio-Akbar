...
- Is bilingual (English/Indonesian), reflecting both an international and a local (Indonesian) audience.
...
- **Language switch** — EN/ID toggle affecting all `data-i18n`-tagged copy site-wide.
...
- **Bilingual parity** — English and Indonesian content must stay equivalent in meaning and completeness; neither locale should lag behind in copy updates.
...
- [ ] Both `en` and `id` copy are updated together if any user-facing text changed.
